...
def iou_calc(A, B):
    ...
    intersection_area = max(0, x_lower_right - x_upper_left + 1) * \
        max(0, y_lower_right - y_upper_left + 1)
...
cv2.imshow("Hightest matching frame", highest_match_frame)
cv2.waitKey(0)
cv2.destroyAllWindows()
print(f'{num_above / len(frames) * 100 :.2f}% of the frames in which iou is larger than {HIGHER_THRESHOLD}')
print(f'{num_below / len(frames) * 100 :.2f}% of the frames in which iou is smaller than {LOWER_THRESHOLD}')
